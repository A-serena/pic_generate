...
lab_he.pack()
HEIBOX = tk.Entry(frame4)
HEIBOX.pack()

lab_wi = tk.Label(frame2, text="横(px)")
lab_wi.pack()
WIGBOX = tk.Entry(frame4)
WIGBOX.pack()

lab_pa = tk.Label(frame2, text="ページ数")
lab_pa.pack()
PAGEBOX = tk.Entry(frame4)
PAGEBOX.pack()

lab_ti = tk.Label(frame2, text="タイトル(半角英数字)")
lab_ti.pack()
TITBOX = tk.Entry(frame4)
TITBOX.pack()

frame3 = tk.Frame(root)
frame3.pack()
LABEL2 = tk.Label(frame3, text="左クリックで生成開始\n右クリックで入力内容の削除")
LABEL2.pack()

BTN1 = tk.Button(frame3, text="生成！")
BTN1.bind("<Button-1>", generate) # 左クリックで入力
BTN1.bind("<Button-3>", DeleteEntryValue) # 右クリックでEntry内を削除
BTN1.pack(side="left")

BTN3 = tk.Button(frame3, text="保存")
BTN3.bind("<Button-1>", a)
BTN3.pack(side="left")

BTN2 = tk.Button(frame3, text="B5サイズ") # ここに表示するもの増えそうだから、何らかのリストにしたい
